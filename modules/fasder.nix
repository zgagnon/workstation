...
in
{
  ...
  options.programs.fasder = {
    enable = mkEnableOption "fasder, a command-line productivity booster";

    package = lib.mkPackageOption pkgs "fasder" { };

    editor = mkOption {
      type = types.str;
      default = "\${EDITOR:-nvim}";
      description = ''
        Editor command to use for opening files.
        Defaults to $EDITOR environment variable or nvim.
      '';
    };
    ...
    fzfCommand = mkOption {
      ...
      default = "fzf";
      description = ''
        Command to use for interactive selection.
        Requires fzf or compatible tool to be installed.
      '';
    };
    ...
    enableAliases = mkOption {
      ...
      default = true;
      description = ''
        Whether to enable default fasder aliases:
        - j: jump to directory
        - v: open file in editor  
        - a: search all (files and directories)
        - d: search directories only
        - f: search files only
      '';
    };

    enableInteractiveSelection = mkOption {
      type = types.bool;
      default = true;
      description = ''
        Whether to enable interactive selection commands (jf, vf).
        Requires fzf or compatible tool.
      '';
    };
    ...
    enableTracking = mkOption {
      ...
      default = true;
      description = ''
        Whether to enable automatic tracking of directory and file access.
        This builds the frecency database that fasder uses.

        For bash/zsh/fish: Uses fasder's built-in hooks to automatically track
        file and directory access from any command.

        For nushell: Uses custom hooks to track directory changes and current directory.
      '';
    };

    enableFileTracking = mkOption {
      type = types.bool;
      default = true;
      description = ''
        Whether to enable automatic file tracking when opening files with editors.

        For nushell: Creates wrapper functions for vi, vim, and nvim that add opened files to the fasder database.
        Note: Cannot wrap nushell's built-in 'edit' command as it cannot be redefined.

        For bash/zsh/fish: Not needed as fasder's built-in hooks (enabled by enableTracking) already handle this.
      '';
    };
