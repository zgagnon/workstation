load-env {
  "HOME_MANAGER_CONFIG": "#($nu.home-path)/workstation/home-manager/"
  "DOOMDIR":"#($nu.home-path)/.doom.dir"
}
$env.HOME_MANAGER_CONFIG = "#($nu.home-path)/workstation/home-manager/"
...
let vcs_info = (do {
  ...
  if $jj_check.exit_code == 0 {
    ...
    if ($branch | is-empty) {
      ...
      load-env {
       "PROMPT_COMMAND": {|| (create_left_prompt) + (char newline)},
